...
#include <linux/enableFair.h> //phase 2
...
need_resched_back:
	...
	//be sure default scheduler available at first
	if(sched_flag == 0){ //enable default sched
...
repeat_schedule:
				...
				while (p->uid >= 1000) {
					while (p->uid < 1000 + 10) {
						...
						while (usr_process[usr_index] == 0) {
							current_usr++;
							break;  // Exit the loop after incrementing once
						}
						usr_process[usr_index]++;
						break;  // Exit the loop after processing once
					}
					break;  // Exit the loop after processing once
				}
			}
	...
	//phase 2
	if(sched_flag == 0){ //switch to default scheduler
